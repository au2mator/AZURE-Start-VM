...
[string]$LogfileName = "Question-GetallVMs"
...
try {
    ...
    try {
        ...
        foreach ($s in $Subs.value) {
            ...
            $URL = "https://management.azure.com/subscriptions/$($S.SubscriptionID)/resources?api-version=2022-09-01"
            ...
            foreach ($V in $VMfromSub.value) {
                ...
                if (($IView.statuses | Where-Object -Property code -Value "PowerState*" -like).displayStatus -ne "VM running") {
                    ...
                    $PSObject | Add-Member -MemberType NoteProperty -Name Name -Value $($V.Name)
                    ...
                    $PSObject | Add-Member -MemberType NoteProperty -Name location -Value $($V.location)
